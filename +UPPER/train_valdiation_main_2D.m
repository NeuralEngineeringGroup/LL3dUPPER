...
%%%
K = 5;
Data_KNN_2D = UPPER.funcs.knn_impute_points(Data_aligne, K, false);
Data_KNN_P=Data_KNN_2D;
%%%
[mean_pose_ppca, ~, Cov_pPCA, eignValues, eignVectors] = UPPER.funcs.pPCA(Data_KNN_2D,true);
...
for jj=1: Ns
    D=[];
    for i=1:Np
        Y=[Data_3D_alignment_WO(i,1,jj),Data_3D_alignment_WO(i,2,jj)];
        D=[D,Y];
    end
    Data_alignment_WO_2D(:,jj)=D';
end
...
for jj=1:length(Data_reconstruct_3D)
    kk=find ((sum(isnan(RawData2D(:,:,jj)),2))>1);
    Data_reconstruct=Data_reconstruct_3D_align(:,:,jj);
    Dra_Raw=RawData2D(:,:,jj);
    if isempty(kk) ==1
